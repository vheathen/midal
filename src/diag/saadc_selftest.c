...
static void make_cfg(const struct adc_dt_spec *sp, struct adc_channel_cfg *out, uint32_t acq_us)
{
...
static void stats_run(uint32_t acq_us)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(chans); i++) {
        ...
        if (err) {
            LOG_ERR("adc_channel_setup ch%u (acq=%uus) failed: %d", chans[i].channel_id, acq_us, err);
            return;
        }
    }
    ...
    for (size_t i = 0; i < ARRAY_SIZE(chans); i++) {
        ...
        for (int k = 0; k < iters; k++) {
            ...
            if (err) { LOG_ERR("read ch%u err=%d", chans[i].channel_id, err); break; }
        ...
        LOG_INF("[acq=%2u us] ch%u: mean=%ld  min=%d  max=%d  p2p=%ld",
                acq_us, chans[i].channel_id, (long)mean, vmin, vmax, (long)p2p);
    }
    ...
    if (ARRAY_SIZE(chans) >= 2) {
        ...
        for (int k = 0; k < 50; k++) {
            ...
            (void)read_once(B, &b);
            (void)read_once(A, &a2);
            if (a1 < 0) a1 = 0; if (a1 > 4095) a1 = 4095;
            if (a2 < 0) a2 = 0; if (a2 > 4095) a2 = 4095;
            int32_t delta = a2 - a1; if (delta < 0) delta = -delta;
